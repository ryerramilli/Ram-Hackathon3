...
$(document).on('click', '#gettyimages_stream_button', showContent);
...
function handleSearch() {
    ...
    port.postMessage({'request' : 'search', 'term' : searchTerm, 'subscriber' : getSubscriberStatus()});
...
function openConnection() {
    ...
    port.postMessage({'request' : 'editor_picks', 'subscriber' : getSubscriberStatus()});
...
function getSubscriberStatus() {
    return document.location.pathname.indexOf('photos_stream') > -1? 'yes' : 'no';
}
...
function attachTopics() {
    ...
    for(var t = 0; t < topics.length; t++) {
        ...
        for(var i=0; i < topics[t].images.length; i++) {
            c += '<img class="gettyimages_editor_picked_image" src="' + topics[t].images[i].thumb + '" clickThru="' + topics[t].images[i].clickThru  + '" data-gettyimages-image-id="' + topics[t].images[i].id + '"/>';
        }
